...
func it_groups_using_truthiness():
	...
	var result = GD__.group_by(collection)
...
func it_groups_using_lambda():
	var result = GD__.group_by(users, func(u): return u.user)
	expect(result).to.equal({
		"barney": [barney],
		"fred": [fred],
		"pebbles":[pebbles]
...
func it_groups_by_using_matches_shorthand():
	var result = GD__.group_by(users, { 'gender': 'm', 'active': true })
	expect(result).to.equal({
		"true": [barney],
		"false": [fred,pebbles]
	})
...
func it_groups_by_using_matches_property_shorthand():
	var result = GD__.group_by(users, ['active', false])
	expect(result).to.equal({
		"true": [fred],
...
func it_groups_by_using_property_shorthand():
	var result = GD__.group_by(users, 'active')
	expect(result).to.equal({
		"true": [barney, pebbles],
		"false": [fred],
	})
