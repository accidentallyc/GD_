...
func it_counts_using_truthiness():
	...
	var result = GD__.count_by(collection)
...
func it_counts_using_lambda():
	var result = GD__.count_by(users, func(u): return u.user)
	expect(result).to.equal({
		"barney": 1,
		"fred": 1,
		"pebbles":1
...
func it_counts_by_using_matches_shorthand():
	var result = GD__.count_by(users, { 'gender': 'm', 'active': true })
	expect(result).to.equal({
		"true": 1,
		"false": 2
	})
...
func it_counts_by_using_matches_property_shorthand():
	var result = GD__.count_by(users, ['active', false])
	expect(result).to.equal({
		"true": 1,
		"false": 2
	})
...
func it_counts_by_using_property_shorthand():
	var result = GD__.count_by(users, 'active')
	expect(result).to.equal({
		"true": 2,
		"false": 1
	})
