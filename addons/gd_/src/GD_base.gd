...
static func _private_constructor(callable:Callable):
...
class __INTERNAL__:
	...
	static var engine_version := Engine.get_version_info()
	
	# for perf reasons
	static var is_version_cache:Dictionary = { }
	
	"""
	The subsection BELOW is dedicated to building the regex used in GD_.words
	"""
	static var rx_words = _private_constructor(func ():
	...
	static func base_is_array(thing, __UNUSED__ = null):
		...
		var max_type = TYPE_PACKED_COLOR_ARRAY
		
		# TYPE_PACKED_VECTOR4_ARRAY == TYPE_PACKED_COLOR_ARRAY + 1
		if is_version(">=4.3"):
			max_type += 1
		return type <= max_type and type >= TYPE_ARRAY
	...
	static func strict_eq(a,b): return a == b
	static func strict_gt(a,b): return a > b
	static func strict_gte(a,b): return a >= b
	static func strict_lt(a,b): return a < b
	static func strict_lte(a,b): return a <= b
	static func strict_ne(a,b): return a != b
	
	static func is_version(version) -> bool:
		assert(version, "Received empty version")
		
		# Cache check
		if is_version_cache.has(version):
			var r = is_version_cache
			return is_version_cache[version]
		...
		match typeof(version):
			...
			TYPE_STRING, TYPE_STRING_NAME:
				...
				match splat:
					["<","=",..]:
						substr_index = 2
						op = __INTERNAL__.strict_lte
					["<",..]:
						substr_index = 1
						op = __INTERNAL__.strict_lt
					[">","=",..]:
						substr_index = 2
						op = __INTERNAL__.strict_gte
					[">",..]:
						substr_index = 1
						op = __INTERNAL__.strict_gt
					["=","=",..]:
						substr_index = 2
					["=", ..]:
						substr_index = 1
				var result = op.call(ver_num, float(version.substr(substr_index)))
				is_version_cache[version] = result # Update cache
				return result
		return false
	
	## Short for call flex
	static func callf(cb, args:Array = _EMPTY_ARRAY_):
		args = Array(args) # Don't mutate original array
		if is_version(">=4.3") and args: # revert to normal callv
			args.resize(cb.get_argument_count())
		return cb.callv(args)
		
	## Short for call compatible (between > v4.3 and < v4.2)
	## This is unique because in GD_ + gdscript 4.2, every argument
	## must be supplied with exactly 2 arguments.
	static func callc(cb, args:Array):
		var count = null
		#region COMPAT<=4.2
		if count == null and is_version("<=4.2"): # revert to normal callv
			count = 2
		#endregion
		if count == null:
			count = cb.get_argument_count()
		args.resize(count)
		return cb.callv(args)
	...
	static func validate_callable(cb, min, max):
