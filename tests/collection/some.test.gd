...
func it_returns_true_for_any_truthy():
    ...
    var result = GD_.some(collection)
...
func it_somes_using_lambda():
    var result = GD_.some(users, func(u,_u): return u.age > 5)
    expect(result).to.be.truthy()


func it_somes_by_using_matches_shorthand():
    var result = GD_.some(users, { 'gender': 'm', 'active': true })
    expect(result).to.be.truthy()


func it_somes_by_using_matches_property_shorthand():
    var result = GD_.some(users, ['active', false])
    expect(result).to.be.truthy()


func it_somes_by_using_property_shorthand():
    var result = GD_.some(users, 'active')
    expect(result).to.be.truthy()
